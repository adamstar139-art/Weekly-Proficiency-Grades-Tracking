...
def save_student_phone_db(student_id, phone):
    sb = get_supabase()
    if sb is None:
        return False
    try:
        sb.table("thaghr_students_info").upsert({"student_id": str(student_id), "phone": str(phone)}).execute()
        st.cache_data.clear()
        return True
    except Exception:
        return False

def delete_student_phone_db(student_id):
    sb = get_supabase()
    if sb is None:
        return False
    try:
...
def render_saudi_header(page_title=""):
    ...
    sub_title_html = f'<div style="font-size: 13px; color: #FFFFFF; font-weight: 600; margin-top:2px;">{page_title}</div>' if page_title else ''
    header_html = f"""
    <div style="
...
##### =========================================================
##### 5. تهيئة حالة الجلسة وقواعد البيانات التفاعلية (Session State)
##### =========================================================
...
##### =========================================================
##### 6. إعدادات الصفحة والتنسيقات المخصصة الشاملة (RTL & Print CSS)
##### =========================================================
st.set_page_config(
...
# تطبيق اتجاه اليمين إلى اليسار (RTL) بشكل كامل وشامل في جميع عناصر Streamlit
st.markdown("""
<style>
/* ضبط الاتجاه العام للتطبيق بالكامل من اليمين إلى اليسار */
html, body, [data-testid="stAppViewContainer"], .main, [data-testid="stHeader"], [data-testid="stSidebar"] {
    direction: rtl !important;
    text-align: right !important;
    font-family: 'Cairo', 'Segoe UI', Tahoma, sans-serif !important;
}

/* محاذاة كافة المدخلات والقوائم المنسدلة من اليمين */
div[data-baseweb="select"], .stTextInput > div, .stNumberInput > div, .stTextArea > div {
    direction: rtl !important;
    text-align: right !important;
}
...
/* قواعد طباعة صارمة تمنع تداخل التقارير وإخفاء عناصر الصفحة */
@media print {
    ...
    .no-print, header, footer, [data-testid="stSidebar"], .stButton, [data-testid="stHeader"] { display: none !important; }
}
</style>
...
if supabase_ready():
    st.sidebar.markdown('<div class="status-badge-ok" style="color:#006C35; font-weight:bold;">🟢 متصل بقاعدة بيانات Supabase الدائمة</div>', unsafe_allow_html=True)
else:
    st.sidebar.markdown('<div class="status-badge-off" style="color:#dc2626; font-weight:bold;">🔴 غير متصل بـ Supabase (يعمل محلياً)</div>', unsafe_allow_html=True)

st.sidebar.markdown("---")
st.sidebar.subheader("📱 إعدادات البوابات والرسائل")

with st.sidebar.expander("💬 إعدادات WhatsApp Direct API"):
    wa_instance = st.sidebar.text_input("Instance ID:", value="", key="sb_wa_instance_key")
    wa_token = st.sidebar.text_input("API Token:", value="", type="password", key="sb_wa_token_key")
    st.caption("💡 إرسال تلقائي مباشر لرسائل الواتساب في الخلفية.")

with st.sidebar.expander("📱 إعدادات Mora SMS"):
    mora_user = st.sidebar.text_input("اسم المستخدم / الرقم:", value="966508634881", key="sb_mora_u_key")
    mora_pass = st.sidebar.text_input("كلمة المرور:", value="THA@0508634881", type="password", key="sb_mora_p_key")
    mora_sender = st.sidebar.text_input("اسم المرسل المعتمد:", value="THAGHR-S", key="sb_mora_s_key")
    mora_otp = st.sidebar.text_input("كود التحقق / OTP:", value="", key="sb_mora_otp_key")
...
##### =========================================================
    ...
    with col_w1:
        weeks = [f"الأسبوع {i}" for i in range(1, 19)]
        selected_week = st.selectbox("اختر الأسبوع:", weeks, key="rep_week_sel")
    with col_w2:
        selected_term = st.selectbox("اختر الفصل الدراسي:", ["الفصل الدراسي الأول", "الفصل الدراسي الثاني"], key="rep_term_sel")
    ...
    if report_type == "📋 التقرير الشامل للمدرسة":
        ...
        with col_g1:
            selected_rep_grade = st.selectbox("اختر المرحلة الدراسية:", list(current_students_db.keys()), key="rep_grade_sel")
        with col_g2:
            available_classes = list(current_students_db[selected_rep_grade].keys())
            selected_rep_class = st.selectbox("اختر الفصل:", available_classes, key="rep_class_sel")
        ...
        if not cat_list:
            ...
            with col_b1:
                with st.expander(f"💬 إرسال واتساب جماعي مباشر ({len(cat_list)} طالب)"):
                    if st.button(f"⚡ إرسال واتساب لـ {cat_name}", key=f"bulk_wa_{cat_name}_key"):
            ...
            with col_b2:
                with st.expander(f"🚀 إرسال SMS جماعي عبر Mora ({len(cat_list)} طالب)"):
                    if st.button(f"⚡ إرسال SMS لـ {cat_name}", key=f"bulk_sms_{cat_name}_key"):
